...
- Writes the one-line `basic/recommended` config entry.
...
Use `update` to replace an older generated config with the stable one-line v3
entry. Adding or removing dependencies no longer requires regenerating it.
...
The `migrate` command remains for moving legacy v1 configs to the v2 composer
shape. For v2 projects, follow the [v2 to v3 migration guide](/guide/migration-v2-to-v3/).
...
For straightforward v1 configs that only import framework packages, `--write`
can replace those imports with the historical v2 bundled-framework boolean
form. It does not finish the v3 dependency migration; after it runs, follow
the [v2 to v3 migration guide](/guide/migration-v2-to-v3/).
...
## Why the Generated Config Is One Line
...
make the generated file stale. Switch to the named `defineConfig()` API only
when the project needs explicit options or local overrides.

## Notes

- The CLI detects frameworks and `doctor` explains missing optional packages.
- Generated files delegate to the documented recommended entry.
